    ...
    <div class="cp-map-card" :class="{ 'cp-map-card--embedded': embedded }">
      <div v-if="!embedded" class="cp-map-header">
        ...
        <p>
          ...
          related to Climate Promise 3 for the selected district(s).
        </p>
      </div>
      ...
      <div id="cp3-map" class="cp-map"></div>
    </div>
  </section>
</template>
...
import { useCP3Map } from "./useCP3Map";
...
const primaryDistrict = computed(() => {
  ...
  if (ids.length) {
    const found = props.districts.find((district) => district.id === ids[0]);
...
const districtLabel = computed(() => {
  ...
  if (ids.length === 1) {
    const district = props.districts.find((item) => item.id === ids[0]);
...
const {
  ...
  loadingProgress,
  refreshSize,
} = useCP3Map(props, primaryDistrict);
