    ...
    <div class="cp-map-card" :class="{ 'cp-map-card--embedded': embedded }">
      <div v-if="!embedded" class="cp-map-header">
        ...
        <p>
          ...
          related to Food Project 1 for the selected district(s).
        </p>
      </div>
      ...
      <div id="home-map" class="cp-map"></div>
    </div>
  </section>
</template>
...
const emit = defineEmits<{
  (e: "loading-state", value: boolean): void;
}>();
...
const {
  ...
  loadingProgress,
  refreshSize,
} = useHomeMap(props, primaryDistrict);
...
watch(loadingProgress, (value) => {
  emit("loading-state", value > 0);
});
</script>
